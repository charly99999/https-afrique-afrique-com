      ...
      {boosted.length > 0 && (
        <section className="animate-fade-up mt-8 px-5">
          <div className="mb-5 flex items-end justify-between">
            <div>
              <span className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-brand-gold">Sélection Premium</span>
              <h2 className="-mt-1 font-display text-3xl italic">Annonces Boostées</h2>
            </div>
            <Link to="/explorer" className="border-b-2 border-brand-green/20 pb-1 text-xs font-bold text-brand-green">Voir tout</Link>
          </div>
      ...
      <section className="mt-10 px-5">
        <div className="relative overflow-hidden rounded-3xl bg-brand-green p-6">
          ...
          <div className="relative z-10">
            <h3 className="mb-2 text-2xl font-extrabold leading-tight text-primary-foreground">Vendez 10× plus vite</h3>
            ...
            <Link to="/abonnements" className="inline-block rounded-xl bg-brand-gold px-6 py-3 text-xs font-bold text-foreground transition active:scale-95">
              Devenir Pro
            </Link>
          </div>
        </div>
  ...
  return (
    <button type="button" onClick={onClick}
      className={`shrink-0 whitespace-nowrap rounded-full px-5 py-2 text-xs font-bold transition ${
        active ? "bg-brand-green text-primary-foreground ring-2 ring-brand-green ring-offset-2 ring-offset-background"
